...
from ..utils import normalize_name, process_uploaded_files  # ✅ process_uploaded_filesを追加！

from ..forms import UploadFileForm  # ✅ 勤務表アップロード用フォームを読み込み

# 🔐 管理者限定ビュー
def staff_required(view_func):
    return user_passes_test(lambda u: u.is_staff)(view_func)

# ✅ 勤務表（Excel）アップロード機能（新）
@staff_required
def upload_shift(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                dialysis_file = request.FILES['dialysis_file']

                output_file = process_uploaded_files(shift_file, dialysis_file)

                response = HttpResponse(
                    output_file.getvalue(),
                    content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
                )
                response['Content-Disposition'] = 'attachment; filename="統合勤務表_更新版.xlsx"'
                return response
    ...
    return render(request, 'shifts/upload_shift.html', {'form': form})

# ✅ 勤務シフトCSVアップロード処理（管理者用・旧）
@staff_required
@csrf_exempt
    ...
    if request.method == 'POST' and request.FILES.get('csv_file'):
        try:
            ...
            if error_logs:
                ...
                messages.warning(request, "一部エラーあり。エラーログダウンロード可能。")
